...
SELECT AlumnoID, Nombre
FROM Alumnos
WHERE Nombre IN ('Romina Quispe','Angel Sanchez');

-- BETWEEN rango de IDs
SELECT AlumnoID, Nombre
FROM Alumnos
WHERE AlumnoID BETWEEN 2 AND 5;
...
SELECT Nombre, 
       CASE 
         WHEN ProfesorID = 2 THEN 'Profesor 2'
         WHEN ProfesorID <> 2 THEN 'Otro Profesor'
...
--Exist para alumnos que si tienen matricula
SELECT a.AlumnoID, a.Nombre
FROM Alumnos a
WHERE EXISTS (
    SELECT 1
...
--Exist para alumnos sin amtricula
SELECT a.AlumnoID, a.Nombre
FROM Alumnos a
WHERE NOT EXISTS (
    SELECT 1
...
--IN
SELECT * FROM Cursos
WHERE Nombre IN ('Historia');


--verificar tablas
SELECT * FROM Matriculas WHERE MatriculaID = 3;
SELECT * FROM Alumnos WHERE AlumnoID = 1;
...
--update con un antes y un despues
BEGIN TRAN;
UPDATE Alumnos
SET Nombre = 'Romina Mamani'
OUTPUT deleted.AlumnoID, deleted.Nombre AS Antes, inserted.Nombre AS Despues
WHERE AlumnoID = 1;
COMMIT;